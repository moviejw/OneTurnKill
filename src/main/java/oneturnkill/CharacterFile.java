...
import oneturnkill.cards.attacks.Strike;
import oneturnkill.cards.attacks.FerociousClaw;
import oneturnkill.cards.attacks.FogClearing;
import oneturnkill.cards.skills.Intuition;
import oneturnkill.cards.skills.Transcode;
import oneturnkill.relics.Reloader;

import java.util.ArrayList;

import static oneturnkill.ModFile.*;

public class CharacterFile extends CustomPlayer {

    static final String ID = makeID("Zaskia");
    ...
    public CharSelectInfo getLoadout() {
        return new CharSelectInfo(NAMES[0], TEXT[0],
                1, 1, 0, 99, 5, this, getStartingRelics(),
    ...
    public ArrayList<String> getStartingDeck() {
        ...
        for (int i = 0; i < 3; i++) {
            retVal.add(FogClearing.ID);
        }
        for (int i = 0; i < 1; i++) {
            retVal.add(FerociousClaw.ID);
        }
        for (int i = 0; i < 2; i++) {
            retVal.add(Transcode.ID);
        }
        for (int i = 0; i < 1; i++) {
            retVal.add(Intuition.ID);
        }
        return retVal;
    }
    ...
    public static class Enums {
        ...
        @SpireEnum
        public static AbstractPlayer.PlayerClass ZASKIA;
        @SpireEnum(name = "ZASKIA_COLOR")
        public static AbstractCard.CardColor ZASKIA_COLOR;
        @SpireEnum(name = "ZASKIA_COLOR")
        @SuppressWarnings("unused")
        public static CardLibrary.LibraryType ZASKIA_LIBRARY_COLOR;
    }
}
